...
tree_data <- readRDS("data/full_run_processed/env_085_30d/envrep_6/runrep_2/tree_data_processed.rds")
env_data <- readRDS("data/full_run_processed/env_085_30d/envrep_6/runrep_2/env_data_processed.rds")
param_data <- readRDS("data/full_run/env_085_30d/envrep_6/runrep_2/run_parameters1afc862843b36.rds")



# 
# tree_data <- readRDS("data/full_run_processed/env_1_0d/envrep_3/runrep_2/tree_data_processed.rds")
# env_data <- readRDS("data/full_run_processed/env_1_0d/envrep_3/runrep_2/env_data_processed.rds")
# param_data <- readRDS("data/full_run/env_1_0d/envrep_1/runrep_3/run_parameters1afc862843b36.rds")
...
env_stress_cat <- env_stress
env_stress_cat$stress <- (1 - env_stress_cat$stress)
env_stress_cat$stress <- env_stress_cat$stress * 365
env_stress_cat$stress_int <- "very_low"
env_stress_cat$stress_int[env_stress_cat$stress > 30] <- "low"
env_stress_cat$stress_int[env_stress_cat$stress > 75] <- "medium"
env_stress_cat$stress_int[env_stress_cat$stress > 105] <- "high"
env_stress_cat$stress_int[env_stress_cat$stress > 150] <- "very_high"
env_stress_cat$stress_int <- as.factor(env_stress_cat$stress_int)
env_stress_cat$stress_int <- factor(env_stress_cat$stress_int, levels = c("very_low", "low", "medium", "high"))
...
file_figure <- tempfile(paste("individual_height", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 240, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")

env_stress_cat$stress_int

colors_for_stress <- rev(foxes_strategy_gradient(4))
colors_for_stress <- colors_for_stress[c(1,2,4,3)]

p <- ggplot(env_stress_cat, aes(time, temp, color = stress_int,  fill = stress_int)) + 
  geom_bar(stat = "identity") + 
  scale_fill_manual("Stress\nIntensity", values = colors_for_stress) +
  scale_color_manual("Stress\nIntensity",values = colors_for_stress) +
  foxes_theme_min + 
  theme(legend.position = "bottom")
p

file_figure <- tempfile(paste("stress", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 300, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
...
file_figure <- tempfile(paste("numbers_of_species", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
...
tree_data_live_ave <- plyr::rbind.fill(lapply(seq_time, function(t){
  dat <- tree_data_live %>% 
    ...
    dplyr::summarise(net_mass_production_dt = mean(net_mass_production_dt, na.rm=TRUE),
              net_mass_production_dt_sd = sd(net_mass_production_dt, na.rm=TRUE)) 
  dat$time = t
  return(dat)
}))
...
p <- ggplot(tree_data_live_ave, aes(x= time, y = net_mass_production_dt)) + 
  geom_line(aes(group = as.factor(species_id * 10 + tree_id), color = as.factor(species_id)), 
            alpha = 0.05) +
  geom_line(data = tree_data_ave, aes(color = as.factor(species_id))) + 
  geom_ribbon(data = tree_data_ave, aes(color = as.factor(species_id), 
                                        fill = as.factor(species_id), 
                                        ymin = net_mass_production_dt - net_mass_production_dt_sd, 
                                        ymax = net_mass_production_dt + net_mass_production_dt_sd), alpha = 0.35, linetype = "blank") + 
  geom_line(data = tree_data_ave_all, color = "#083855", alpha = 0.75) + 
  geom_ribbon(data = tree_data_ave_all, aes(ymin = net_mass_production_dt - net_mass_production_dt_sd, 
                                            ymax = net_mass_production_dt + net_mass_production_dt_sd), 
              alpha = 0.35, linetype = "blank") + 
  scm_ribbon + 
  sfm + 
  species_wrap +
  scale_x_continuous("Time (yrs)") +
  scale_y_continuous("Individual Plant Net Mass Production Rate (kgCyr^-1/plant)") +
  foxes_theme +
  theme(legend.position = "bottom")
p

file_figure <- tempfile(paste("individual_NPP", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 240, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
...
tree_data_live_ave <- plyr::rbind.fill(lapply(seq_time, function(t){
  dat <- tree_data_live %>% 
    dplyr::filter(time > (t-1) & time < (t+1)) %>%
    dplyr::group_by(species_id, tree_id) %>%
    dplyr::summarise(dbiomass_dt = mean(dbiomass_dt, na.rm=TRUE),
                     dbiomass_dt_sd = sd(dbiomass_dt, na.rm=TRUE)) 
  dat$time = t
  return(dat)
}))
...
p <- ggplot(tree_data_ave, aes(x= time, y = dbiomass_dt)) + 
  # geom_line(aes(group = as.factor(species_id * 10 + tree_id), color = as.factor(species_id)), 
  #           alpha = 0.05) +
  geom_line(data = tree_data_ave, aes(color = as.factor(species_id))) + 
  geom_ribbon(data = tree_data_ave, aes(color = as.factor(species_id), 
                                        fill = as.factor(species_id), 
                                        ymin = dbiomass_dt - dbiomass_dt_sd, 
                                        ymax = dbiomass_dt + dbiomass_dt_sd), alpha = 0.35, linetype = "blank") + 
  geom_line(data = tree_data_ave_all, color = "#083855", alpha = 0.75) + 
  geom_ribbon(data = tree_data_ave_all, aes(ymin = dbiomass_dt - dbiomass_dt_sd, 
                                            ymax = dbiomass_dt + dbiomass_dt_sd), 
              alpha = 0.35, linetype = "blank") + 
  scm_ribbon + 
  sfm + 
  species_wrap +
  scale_x_continuous("Time (yrs)") +
  scale_y_continuous("Individual Growth Rate (kgCyr^-1/plant)") +
  foxes_theme +
  theme(legend.position = "bottom")
p

file_figure <- tempfile(paste("individual_biomass_dt", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 240, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
...
tree_data_ave <- plyr::rbind.fill(lapply(seq_time, function(t){
  dat <- tree_data_live %>% 
    ...
    dplyr::summarise(net_mass_production_dt = sum(net_mass_production_dt/100, na.rm=TRUE)) %>%
    dplyr::group_by(species_id) %>%
    dplyr::summarise(net_mass_production_dt = mean(net_mass_production_dt, na.rm=TRUE),
                     net_mass_production_dt_sd = sd(net_mass_production_dt, na.rm=TRUE)) 
  dat$time = t
...
p <- ggplot(tree_data_ave, aes(x= time, y = net_mass_production_dt)) + 
  # geom_line(aes(group = as.factor(species_id * 10 + tree_id), color = as.factor(species_id)), 
  #           alpha = 0.05) +
  geom_line(data = tree_data_ave, aes(color = as.factor(species_id))) + 
  geom_ribbon(data = tree_data_ave, aes(color = as.factor(species_id), 
  ...
  species_wrap +
  scale_x_continuous("Time (yrs)") +
  scale_y_continuous("Total Plant Net Mass Production Rate (kgCyr^-1m^-2)") +
  foxes_theme +
  theme(legend.position = "bottom")
p

file_figure <- tempfile(paste("summed_NPP", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 240, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")


tree_data_ave_percent <- merge(tree_data_ave, tree_data_ave_all, by = "time") %>%
  mutate(net_mass_production_percent = net_mass_production_dt.x/net_mass_production_dt.y)

p <- ggplot(tree_data_ave_percent, aes(fill=as.factor(species_id), color=as.factor(species_id), y=net_mass_production_percent * 100, x=time)) + 
  geom_bar(stat="identity") + 
  sfm + 
  scm + 
  scale_x_continuous("Time (yrs)") +
  scale_y_continuous("Percent total Net Mass Prdouction Rate") +
  foxes_theme +
  theme(legend.position = "bottom")
p

file_figure <- tempfile(paste("percent_NPP", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 240, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
...
tree_data_ave <- plyr::rbind.fill(lapply(seq_time, function(t){
  dat <- tree_data_live %>% 
    dplyr::filter(time > (t-1) & time < (t+1)) %>%
    dplyr::group_by(species_id, time) %>%
    dplyr::summarise(dbiomass_dt = sum(dbiomass_dt/100, na.rm=TRUE)) %>%
    dplyr::group_by(species_id) %>%
    dplyr::summarise(dbiomass_dt = mean(dbiomass_dt, na.rm=TRUE),
                     dbiomass_dt_sd = sd(dbiomass_dt, na.rm=TRUE)) 
  dat$time = t
...
p <- ggplot(tree_data_ave, aes(x= time, y = dbiomass_dt)) + 
  ...
  geom_ribbon(data = tree_data_ave_all, aes(ymin = dbiomass_dt - dbiomass_dt_sd,
                                            ymax = dbiomass_dt + dbiomass_dt_sd),
        alpha = 0.35, linetype = "blank", color = "#083855") +
  scm_ribbon + 
  sfm + 
  species_wrap +
  scale_x_continuous("Time (yrs)") +
  scale_y_continuous("Total Plant Growth Rate (kgCyr^-1/100m2)") +
  foxes_theme +
  theme(legend.position = "bottom")
p

file_figure <- tempfile(paste("summed_growth", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 240, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
...
tree_data_ave_percent <- merge(tree_data_ave, tree_data_ave_all, by = "time") %>%
  mutate(dbiomass_dt_percent = dbiomass_dt.x/dbiomass_dt.y)

p <- ggplot(tree_data_ave_percent, aes(fill=as.factor(species_id), color=as.factor(species_id), y=dbiomass_dt_percent * 100, x=time)) + 
  geom_bar(stat="identity") + 
  sfm + 
  scm + 
  scale_x_continuous("Time (yrs)") +
  scale_y_continuous("Percent total Growth Rate") +
  foxes_theme +
  theme(legend.position = "bottom")
p

file_figure <- tempfile(paste("percent_growth", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 240, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
...
tree_data_live_ave <- plyr::rbind.fill(lapply(seq_time, function(t){
  dat <- tree_data_live %>% 
    ...
    summarise(storage_portion = mean(storage_portion, na.rm=TRUE),
...
file_figure <- tempfile(paste("storage_portion_ind", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 190, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
...
tree_data_live_ave <- plyr::rbind.fill(lapply(seq_time, function(t){
  dat <- tree_data_live %>% 
    filter(time > (t-1) & time < (t+1)) %>%
    group_by(species_id, tree_id) %>%
    summarise(mass_storage = mean(mass_storage, na.rm=TRUE),
...
file_figure <- tempfile(paste("storage_mass_ind", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 240, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
...
file_figure <- tempfile(paste("mortality_likelihood_growth", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 190, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
...
file_figure <- tempfile(paste("mortality_likelihood", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 190, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
...
file_figure <- tempfile(paste("height", "canopy", sep = "_"), tmpdir = "out", fileext = ".png")
ggsave(file_figure, plot = p, device = NULL, path = NULL,
       scale = 1, width = 190, height = 138, dpi = 600, limitsize = TRUE,
       units =  "mm")
